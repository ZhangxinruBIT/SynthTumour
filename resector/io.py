...
CHECK_QFAC = False
debug_dir = None#'/data7/xinruzhang/RESECTION/HCP_now/100307/debug'
debug_num_files = 0
...
def save_debug(x):
    ...
    if isinstance(x, vtk.vtkPolyData):
        # path = debug_dir / f'{out_stem}.vtp' xinxinzhang
        path = os.path.join(debug_dir,out_stem+'.vtp')
        write_poly_data(x, path, flip=True)
    elif isinstance(x, sitk.Image):
        #path = debug_dir / f'{out_stem}.nii.gz' xinxinzhang
        path = os.path.join(debug_dir,out_stem+'.nii.gz')
        write(x, path)
    else:
